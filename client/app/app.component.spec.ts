...
describe('Component: App', () => {
  ...
  it('should display the navigation bar correctly for guests', () => {
    ...
    expect(de[2].nativeElement.textContent).toContain('Students');
    expect(de[3].nativeElement.textContent).toContain('Login');
    expect(de[4].nativeElement.textContent).toContain('Register');
    expect(de[0].attributes['routerLink']).toBe('/');
    expect(de[1].attributes['routerLink']).toBe('/cats');
    expect(de[2].attributes['routerLink']).toBe('/students');
    expect(de[3].attributes['routerLink']).toBe('/login');
    expect(de[4].attributes['routerLink']).toBe('/register');
  });
  ...
  it('should display the navigation bar correctly for logged users', () => {
    ...
    expect(de[0].attributes['routerLink']).toBe('/');
    expect(de[1].attributes['routerLink']).toBe('/cats');
    expect(de[2].attributes['routerLink']).toBe('/sudents');
